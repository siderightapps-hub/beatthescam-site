...
> **Last touched:** 2026-07-10 (later same day) — **"Action Fraud" → "Report Fraud" rebrand COMPLETE end-to-end** (the UK service rebranded Dec 2025; full launch Jan 2026). Canon/`build.py`/gate/generator templates landed in `41c8820ec`, and the guide body/FAQ prose sweep landed as `f70802f41` (PR #56, merged after the diversification entry below was written) — every rendered field in `posts.json` + `category-hubs.json` is now "Report Fraud"-branded and `dist/` carries zero `actionfraud.police.uk` links; the only remaining "Action Fraud" strings sit in the dead, never-rendered legacy `content` field (harmless). Write **"Report Fraud"** in all new content; "Action Fraud" only as a former-name parenthetical. Same session: **full docs refresh** — CLAUDE.md rewritten around a critical-invariants structure, stale counts (189/183/~190 → 181), the pre-rebrand "sweep still open" notes, and outreach-template branding all corrected; `.env.example` now also documents `GOOGLE_SEARCH_CONSOLE_TOKEN` + `DIGEST_TO`. Next focus unchanged — see the entry below (DMARC ramp, outreach monitoring, heading-scan on any off-template newcomers).
>
