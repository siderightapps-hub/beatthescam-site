...
> **▶ RESUME (handoff 2026-06-29):** **Batches 1–8 are all merged & live (41 diversified pages, PRs #12–#25)** plus the fact-checked `fake-travel-agent-scam-uk` cron guide (PR #20). The original **§3 top-30 work-list is cleared** (batches 1–6); **batches 7–8 were the first AdSense-breadth tranche beyond it** — page-1 generic-outline leakers picked by cross-referencing the ~140 still-generic pages against GSC/Bing impressions (vertical-diverse). Batch 8 also reframed `viagogo-scam-uk` from a "clone-site scam" page to a platform-legitimacy guide (CMA 2018 court order, drip pricing, Section 75). Corpus is **190 guides** (batch 6 consolidated the duplicate DVLA email pair: `dvla-email-scam-car-tax` → 301 → `dvla-email-scam-uk`). **Next:** re-pull `gsc_report.py` (~mid-July) to measure how batches 1–8 perform, then pick the next tranche the same way. ⚠️ **Build gotcha (batch 8):** never run concurrent `build.py` — the `rmtree` race corrupted an intermediate commit; wait for a single build to finish before `git add dist/`.
