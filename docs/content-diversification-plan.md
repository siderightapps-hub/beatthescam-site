...
**Completed (11 pages):** Batch 1 (PR #12, merged+live) — `halifax-bank-scam-text-uk`, `lloyds-bank-scam-text`, `dvla-vehicle-tax-text-scam`, `natwest-fraud-text-message-scam-uk`, `barclays-bank-text-scam-uk`, `santander-email-scam-uk` (+ Barclays `.example` hotfix PR #13). Batch 2 (PR #14) — `amazon-order-scam-email-checklist`, `hermes-parcel-scam-text-uk`, `royal-mail-text-scam-uk`, `hmrc-tax-refund-text-scam-uk`, `evri-delivery-scam-guide`. **+ global:** `seo_title` suffix-drop on guides; canon `report_label` → "Report Fraud (Action Fraud)"; renderer mixed-prose+bullets + inline-`code`; `faq_schema`/`howto_schema` strip code backticks (no JSON-LD leak); category stragglers `Email/Text Message Scams` → `email`/`sms`.
