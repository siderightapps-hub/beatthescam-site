...
> **▶ RESUME (handoff 2026-06-29):** Batches 1–5 are **merged & live** (26 pages) plus the fact-checked `fake-travel-agent-scam-uk` cron guide (PR #20). **Batch 6 is fact-checked & PR'd — [PR #23](https://github.com/siderightapps-hub/beatthescam-site/pull/23), awaiting merge** (5 pages, gate 0/0, build-verified; operator-fact-checked + independently re-checked): `whatsapp-family-emergency-scam` ('Hi Mum' new-number con), `military-romance-scam-uk`, `dvla-email-scam-uk` (**consolidated** — absorbs `dvla-email-scam-car-tax` via 301), `autotrader-car-scam-uk`, `amazon-delivery-scam-text-uk`. Note the plan's old tail slugs were approximate: `whatsapp-scam-family-message-uk` is a 301 → `whatsapp-family-emergency-scam`, and `dvla-scam-email-awareness` was a category redirect (never a page) — the real DVLA email targets were the duplicate pair, now consolidated. **This clears the §3 top-30 work-list.** Next, if continuing: re-pull `gsc_report.py` (token may need refresh) to pick the next tranche by impressions, or move to the de-templating-for-AdSense-breadth tail beyond the top 30.
