...
> **▶ RESUME (handoff 2026-06-28):** Batches 1–4 are **merged & live** (21 pages) plus the fact-checked `fake-travel-agent-scam-uk` cron guide (PR #20, merged). **Batch 5 is drafted** (5 pages, gate 0/0, build-verified) and committed as a **WIP on branch `content/diversification-batch5`** (pushed to origin) — *awaiting operator fact-check*: `bank-transfer-scam-uk` (the "safe account" APP con), `hmrc-tax-rebate-email-scam`, `recruitment-agency-scam-uk`, `data-entry-job-scam-uk`, `universal-credit-scam-uk` (the advance-loan scam). The readable `docs/review/` working-docs were **removed from the repo and git-ignored** (now local-only, regenerable) — to fact-check batch 5, regenerate the 5 review files from `posts.json`, hand them over, apply the `-c.md` corrections, build, then open the batch-5 PR. **Remaining tail after batch 5:** `whatsapp-scam-family-message-uk`, `military-romance-scam-uk`, `dvla-scam-email-awareness`, `autotrader-car-scam-uk`, `amazon-delivery-scam-text-uk`.
...
- **2026-06-28 — Batch 5 drafted (WIP, not yet merged) + review-docs removed.** Drafted 5 Tier-3 pages — `bank-transfer-scam-uk`, `hmrc-tax-rebate-email-scam`, `recruitment-agency-scam-uk`, `data-entry-job-scam-uk`, `universal-credit-scam-uk` (gate 0/0, build verified) — committed as a WIP on `content/diversification-batch5` (pushed), awaiting operator fact-check. Same recipe: bespoke outlines, `.example` specimens, APP figures, website-checker + sibling cross-links, §7 7726 kept off NCSC attribution. Carried the Indeed DBS-fee correction into both jobs pages; `universal-credit` centres the **advance-loan scam** (a fake agent claims a UC advance — a real loan repaid from benefits — in your name). **Separately:** the `docs/review/` readable working-docs (44 committed `.md`/`-c.md`) were **removed and the folder git-ignored** (chore PR) — they were local QA artifacts, never site content; regenerate locally per batch going forward.
