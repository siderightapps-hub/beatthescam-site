...
- The site has 185 indexable UK-focused guides, a mature technical SEO foundation, schema markup, original public research (including the UK Scam Statistics dataset) and a secured AI scam checker.
...
├── .github/
│   └── workflows/
│       ├── daily-publish.yml          # Content pipeline (Tue/Fri 05:07 UTC) — opens a review PR
...
|---|---|---|
| **Google AdSense** | **Rejected — "Low value content" policy violation** (operator-supplied dashboard screenshot, 2026-08-16; site ownership still verified). Google's "not ready to show ads" page cites minimum-content-requirements/thin-content/quality-guidelines links. Supersedes the 11 August "Getting ready" state. | Investigate before resubmitting — see Section 20 gotcha on this rejection; do not blind-resubmit without a concrete change |
...
- Runs at **05:23 UTC** on Tue/Fri (daily until 2026-07-23; was 06:30 UTC until 2026-05-22). Off popular slots to reduce delay; queued behind `daily-publish` via shared `concurrency: content-pipeline` group.
...
- **Quick answer box on every guide (added 2026-07-25)** — a page-specific verdict-first 35–60-word `quick_answer` field rendered above the Key rule notice, with `SpeakableSpecification` schema pointing AI/voice assistants at it (`render_post` + `speakable_schema` in `build.py`). Operator rule: when a guide's content changes, regenerate/recheck its quick answer rather than carrying it forward.
- **Original citable data (added 2026-07-25)** — `/research/uk-scam-statistics/`: 28 live-verified official records (UK Finance, PSR, ONS, Cifas, FCA, Ofcom, GASA, Citizens Advice) with per-record geography labels, CSV+JSON downloads and Dataset JSON-LD (CC BY 4.0). Includes an original explainer of the suspended police-recorded fraud series during the Action Fraud → Report Fraud migration. Refresh quarterly (see Section 19).
...
Structure: a brief site description, then a "Research and datasets" section (research index, methodology, each visibility report, and the UK Scam Statistics page), then categories listed (with guide counts), then every guide grouped by category with title + URL + description, then an "Optional" section for About/Contact/Privacy. Line count will drift with corpus size — check `dist/search.json` for the exact current figure (185 entries as of 2026-07-25). A companion `llms-full.txt` carries the complete guide corpus inline; the statistics dataset is downloadable at `/research/data/uk-scam-statistics.csv` and `.json`.
...
- Refresh `/research/uk-scam-statistics/` (next: **October 2026**): re-verify every record in `content/uk-scam-statistics.json` against its primary source, fold in new releases (PSR dashboard ~quarterly; ONS quarterly; UK Finance each June; Cifas Fraudscape each March; GASA/Cifas each November), bump `updated`, rebuild. At the same time re-verify date-sensitive guide claims flagged by the operator (Halifax→Lloyds transition wording, 159 participant list). Note: ukfinance.org.uk and cityoflondon.police.uk block scripted fetches — verify those in a real browser.
...
19. **GSC OAuth testing-token churn — RESOLVED 2026-08-27.** Google expires refresh tokens after ~7 days when an OAuth consent screen is in "Testing". The actual Beat The Scam project (`beatthescam`, client IDs beginning `758467619755…`) is now confirmed **In production**; its local token refreshed successfully on 2026-08-27 and the scheduled Search Console workflow completed successfully on 2026-08-25. `daily-search-console.yml` still fails loudly if authentication breaks rather than silently skipping generation. **Recovery only if needed:** run `python3 scripts/auth_google.py`, then update the `GOOGLE_SEARCH_CONSOLE_TOKEN` CI secret. Production refresh tokens can still be revoked or expire after long inactivity, so retain the failure alert; do not describe them as rotating weekly.

20. **Check GSC demand before deleting pages for AdSense.** The 2026-05-24 "low value content" purge 301'd the site's HIGHEST-demand URLs (`dpd-delivery-scam-text` = 1,905 impr / pos 10.2, plus yodel/ups) to a thin category page, collapsing the courier cluster within ~2 weeks (resurrected 2026-06-05 via `scripts/recover_courier_guides.py`). The sin was *thinness* (<300-word stubs), not the topic — pull `scripts/gsc_report.py` before purging anything. **UPDATE 2026-08-16 — the recovery restored the page but never restored the ranking, ~2.5 months later.** A day-by-day GSC pull for `dpd-delivery-scam-text` over the trailing 180 days shows a clean signature of the purge/recovery cycle: steady ~pos 10, up to 540 impressions/day through 2026-05-20; a **total blackout** (0 impressions on almost every day) from 2026-05-21 through 2026-06-13 — matching the redirect-then-restore window almost exactly; then the page reappears from ~2026-06-14 onward but has never recovered better than position ~24, mostly sitting in the 30s–60s since, versus its pre-purge position ~10. Impressions are down from a 1,905/90-days peak to 303/90-days now, with **zero clicks** at the current position. The technical setup on the live page is clean (correct canonical, `index,follow`, 17 internal links already pointing to it) — this reads as a genuine, lasting authority/ranking loss from having been temporarily 301'd away, not a current on-page or technical defect. The same pattern shows on the other two courier guides rescued the same day: `yodel-scam-text-messages` (pos ~19.6, was likely page-1) and `ups-delivery-scam-text-messages-uk` (pos ~25.8); `evri-delivery-scam-guide`, which was NOT part of the May purge, is still healthy at pos ~10.5 for comparison. **Lesson: a 301-then-restore at the same URL does not reliably restore prior ranking** — treat a page pulled from the index (even briefly, even by your own script) as having lost real authority that has to be earned back (internal links, fresh signals, possibly a GSC URL-inspection request), not as a solved problem once the content is live again.
...
31. **`@netlify/blobs` is pinned `~10.1.0` ON PURPOSE — don't bump to 10.2.0+ (2026-06-25).** The functions' atomic counters/tokens use the conditional-write API (`onlyIfMatch`/`onlyIfNew`/`setJSON`→`{modified}`), which **only exists in v10+** — under the old `^8.1.0` pin (resolving to 8.2.0, where `setJSON`→`void` and `SetOptions={metadata?}`) every CAS was a **silent no-op** (rate limits, spend cap, send caps, single-use confirm tokens all degraded to last-write-wins). `~10.1.0` is the newest version with the CAS API but BEFORE `@netlify/otel` enters (at 10.2.0), which pulls an OpenTelemetry chain with **6 moderate `npm audit` vulns**. So `~10.1.0` = CAS API + `npm audit` clean (34 pkgs). If Dependabot proposes ≥10.2.0, **decline** until Netlify fixes the upstream OTel vulns. 10.1.0 ships CJS + Node≥16, so no function-code change was needed. **UPDATE 2026-08-06 — the pin stands, but the ORIGINAL reason above is now obsolete; do not re-derive it and conclude the bump is safe.** Dependabot PR #87 (`10.1.0` → `10.7.11`) was declined and closed on new evidence. What was re-checked: (a) the OTel vulns **are fixed** — `npm audit` reports **0 vulnerabilities** for *both* `10.1.0` and `10.7.11` (55 deps), so the "6 moderate vulns" gate written above no longer applies; (b) the CAS API is **fully intact** in `10.7.11` — `onlyIfNew`, `onlyIfMatch`, `setJSON`→`Promise<WriteResult>` with `WriteResult.modified: boolean`, plus `getWithMetadata`/`etag`, all matching how `check-scam.js`/`subscribe.js`/`confirm-subscribe.js` actually call them. **The blocker is now the deploy itself:** the Netlify deploy preview **fails** on the bump, and that deploy is what bundles the five functions. It is specific to this change, not an outage — on the same repo and the same day, PR #88 (pip bump) and PR #93 (content) both deployed clean while #87 failed (Netlify deploy `6a705d51d6ed2a0008945ac5`). One likely factor, unconfirmed without Netlify's build logs: the OTel chain takes the installed footprint from **5.0M / 42 packages to 32M / 62 packages**, and every function bundles it. **So: still decline ≥10.2.0 — but the test to re-run is "does the deploy preview go green", not "is `npm audit` clean".** Both audits being clean is exactly what would tempt a future session into merging a bump that breaks the functions.
...
38. **Audit-agent measurements need deterministic re-verification before acting on them (learned 2026-07-24/25, second full-audit session).** Three near-misses in one session: (a) the technical-SEO agent reported 4 guides at ~300 words — a measurement error; recomputing from `sections`+`faq` gave 835–1,119 words, so "thicken 4 guides" was mostly a no-op (always recount with `python3` over posts.json before commissioning rewrites); (b) a thickener agent reported a "live rendering bug" (bracket link rendering literally) that was false — see gotcha 34's update; (c) two overlapping stats researchers disagreed on the last police-recorded fraud figure (1.2m YE Jun 2025 vs 1.4m YE Sept 2025) — the second, which re-verified the first's claims, was right, and the wrong figure was already rendered into the working-tree page before being caught. Rule of thumb: agent *findings* are leads; only deterministic local checks or a second independent verification make them facts. Related win: the fan-out pattern (12 fact-check agents over 16 guides each + 3 site agents) audited the whole corpus in one session; the whole remediation shipped at `5b935f85e`.

39. **A hand-edit changed `content/posts.json`'s trailing byte and silently desynced the file from the generator that writes it (found 2026-08-06, fixed same day).** For its entire history the file ended with `]` and **no** trailing newline, and invariant 5 in `AGENTS.md`/`CLAUDE.md` documented exactly that — accurately. Inside PR #90 the bot's own generation commit (`6975dcbf5`) still ended `5d`; the operator-review correction commit that followed (`448367e25`) added a trailing `0a`, and that shipped to `main`. The next session (PR #91) followed the still-accurate-the-day-before instruction literally, wrote `json.dumps(...)` with no newline, and got a spurious `\ No newline at end of file` diff on the last line **on top of** its intended edit, corrected by hand. **Resolution: the trailing newline is now canonical**, because it was already the house convention everywhere else — every other `content/*.json` carries one, and five of the seven `posts.json` writers already appended it. The two that did not were both live cron paths and were patched: `generate_content_claude.save_posts()` (daily-publish) and `search_console_articles.py`'s save block (daily-search-console). Correcting only the prose would have been unstable — the next Tue/Fri cron would have stripped the newline again and made the invariant false within days. **Two general rules:** (a) when a hand-edit and a generator disagree about a file's exact bytes, **fix the generator, don't just re-document the file** — a whitespace-only divergence is invisible in review and resurfaces as a phantom diff in the *next* session's PR, attributed to whoever happens to touch the last line; (b) **verify a byte-exactness claim by reproducing it** (`hashlib.sha256` over `json.dumps(...) + "\n"` vs the file on disk), never by trusting the docs — this claim was true when written and went stale one commit later.

40. **AdSense rejected the site for "Low value content" on 2026-08-16, reversing the 11 August "Getting ready" state (operator-supplied dashboard screenshot; not yet independently re-verified from the live dashboard).** Google's rejection page cites its minimum-content-requirements/thin-content/quality-guidelines links generically — it gives no page-level detail, so the cause is inferred, not confirmed. What's already ruled out: the readiness checklist (privacy/about/contact/cookies pages, `ads.txt` Authorised, robots.txt not blocking `Mediapartners-Google`) all still hold, and the July 2026 content-diversification project already de-templated all 186 guides off the generic 6-section outline that a prior audit flagged as an AdSense scaled-content risk (gotcha in the July history above) — so this is not the same failure recurring. Two things a 2026-08-16 investigation found that hadn't been checked before: (a) a new corpus-average heuristic scan (`scripts/check_ai_writing_style.py`) shows heavy em-dash usage (13–24 per guide on the worst offenders) and LLM-cliche words ("delve", "underscore", "unlock") across dozens of guides — a writing-style signal nothing in the pipeline checked before, since `content_gate.py` verifies facts, not prose style; (b) live word counts run 317–3,137 (median 814), with 41/186 guides under 600 words — not "thin" by the ~300-word bar but light for YMYL fraud-advice content. Also relevant: the 10 August Ahrefs baseline shows DR 2 and "no measured organic traffic/keywords," which is the kind of low-authority/low-engagement signal Google's automated low-value classifier is known to weigh alongside the content itself. **Do not guess-and-rewrite content to chase this flag** — the 2026-05-24 "low value content" purge (gotcha above) shows that reacting to an unspecific low-value signal by deleting/rewriting without GSC-demand evidence cost the site its highest-demand pages. **That GSC pull happened same-day (2026-08-16) and is decisive: trailing 90 days, sc-domain property, 2,886 impressions but only 14 clicks (0.49% CTR) at an average position of 37.0** — that's page 4 of results, essentially invisible. The top-impression page (`/guides/dpd-delivery-scam-text/`, 303 impressions) has **zero** clicks; the whole top-30-by-impressions table shows near-total zero-click rows. This is the same near-zero-organic-traffic picture the 10 August Ahrefs baseline (DR 2, "no measured organic traffic/keywords") already implied, now confirmed first-party. **Conclusion: the dominant signal behind the AdSense rejection is very likely near-zero real-world search engagement/authority, not a specific on-page content defect** — hundreds of gated, fact-checked, de-templated guides are producing almost no clicks because they rank around position 30-90 for their target queries, and a site nobody clicks through to reads as "not providing value" to Google's automated classifier regardless of what's actually on the page. This reframes the fix: growing genuine rankings/CTR (backlinks, the near-miss-query opportunities `gsc_report.py` already surfaces, title/meta tuning on the zero-click queries) is the real lever, not further content rewrites — the AI-writing-style cleanup is still worth doing but shouldn't be mistaken for the fix. Do not resubmit to AdSense until this improves and there's a fresh, better GSC pull to point to.
...
- **Quick answer boxes shipped on all 185 guides** (`quick_answer` field + `speakable` schema; operator-approved as a batch). Operator standing rule: regenerate a guide's quick answer whenever its content changes.
- **`/research/uk-scam-statistics/` shipped** — original quarterly-refresh dataset (28 live-verified records, geography-labelled, CSV/JSON, Dataset schema, CC BY 4.0), including the suspended-recorded-fraud-series explainer. Next refresh October 2026 (Section 19).
- **Guide expansions** (operator-approved): dvla-vehicle-tax-text-scam, nhs-appointment-scam-text-uk, halifax-bank-scam-text-uk (incl. the Halifax→Lloyds brand-change scam hook — announced 2 Jul 2026, closed to new customers 16 Jul 2026). NPA ad mode extended to advance-fee/pharmacy guides. Corpus now 185/185 `sources_checked`.
